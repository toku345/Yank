...
`./scripts/build-production.sh` は例外的に、`Release` archive を作るそのコマンドにだけ
`-xcconfig SupportingFiles/Base.xcconfig` を渡す。これにより、通常の Release/archive に
個人設定を波及させず、ローカルインストール用の最適化ビルドだけを Apple Development
証明書で署名する。同時に debugger entitlement の注入を無効化し、署名・hardened
runtime・entitlements・version・architecture をインストール前に検証する。

```bash
# archive のみ生成
./scripts/build-production.sh

# /Applications/Yank.app へ安全に置換して起動
./scripts/install-production.sh
```

Apple Development 署名は自分の Mac への安定したインストール用であり、第三者配布用ではない。
公開リリースには Developer ID Application 署名と notarization が必要。詳細は
`docs/releasing.md` を参照する。
