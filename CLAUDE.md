...
このファイルは Claude/Codex などのエージェント共通の作業メモです。
`AGENTS.md` はこの `CLAUDE.md` への symlink として管理してください。
...
- Phase 3 でセンシティブな値の扱い、ステータスバー、ログイン起動、検索を検討する。
...
- `project.yml` 変更時、Swift ファイルの新規追加・削除時は `xcodegen generate` を実行します。
...
- commit message を作成・編集する場合は、末尾に次の trailer を一度だけ含めます。
...
CI は GitHub Actions の `macos-15` ランナーで PR と `main` push 時にビルド・テストを実行します。
...
設計判断は `docs/adr/NNNN-<slug>.md` に記録します。
