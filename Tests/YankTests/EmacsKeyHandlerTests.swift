...
final class EmacsKeyHandlerTests: XCTestCase {
    ...
    func testEscape_closes() {
        ...
        XCTAssertEqual(EmacsKeyHandler.action(event: event), .close)
    }

    func testDelete_deletesSelectedItem() {
        let event = makeKeyEvent(keyCode: 51)
        XCTAssertEqual(EmacsKeyHandler.action(event: event), .deleteSelected)
    }

    func testRepeatedDelete_returnsNil() {
        let event = makeKeyEvent(keyCode: 51, isARepeat: true)
        XCTAssertNil(EmacsKeyHandler.action(event: event))
    }
    ...
    func testDownArrow_movesDown() {
        ...
        XCTAssertEqual(EmacsKeyHandler.action(event: event), .move(.down))
    }

    func testUpArrow_movesUp() {
        let event = makeKeyEvent(keyCode: 126)
        XCTAssertEqual(EmacsKeyHandler.action(event: event), .move(.up))
    }
    ...
    func testUnhandledControlKey_returnsNil() {
        ...
        XCTAssertNil(EmacsKeyHandler.action(event: event, trackedModifiers: .control))
    }
